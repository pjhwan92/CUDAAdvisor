...
namespace{
	...
	struct instru_host_sig : public ModulePass{
		...
		virtual bool runOnModule(Module &M)
		{
			...
			Type* arrMallocManaged[4] = {Type::getInt8PtrTy(C), Type::getInt64Ty(C), Type::getInt8Ty(C), NULL};
			...
			M.getOrInsertFunction("cudaMallocManaged", typeMallocManaged);
		...
		virtual bool runOnBasicBlock73(Function::iterator &BB)
		{
			...
			for(BasicBlock::iterator BI = BB->begin(), BE = BB->end(); BI != BE; ++BI)
			{
				...
				if ( auto *op = dyn_cast<InvokeInst>(BI) )
				{
					...
					builder.CreateCall( fcudamalloc, { buffer_d, builder.getInt64(BUFFERSIZE)} );
